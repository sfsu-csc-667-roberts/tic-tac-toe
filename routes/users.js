  ...
  bcrypt.hash(password, 10)
    ...
    .then(user => {
      request.login({ id: user.id }, error => {
        if (error != null) {
          console.log(error)
          response.render('unauthenticated/login', { message: ['An error occurred when trying to log you in.'] })
        } else {
    ...
    .catch(error => {
      console.log(error)
      response.render('unauthenticated/register', { message: ['Something bad happened.' + error] })
    })
})
